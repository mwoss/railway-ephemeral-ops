...
import {useAbortMission, useMissionHistory, useSaveMissionToHistory, useStartMission, useUpdateMission} from './useMissions';
...
export function useMissionManager() {
  ...
  const handleStartMission = async (image: string, command: string, ttl: number | null): Promise<boolean> => {
  ...
  const archivedMissions = missions.filter(
    (m) => m.status === 'expired' || m.status === 'failed' || m.status === 'terminated' || m.status === 'cleanup_failed'
  );
